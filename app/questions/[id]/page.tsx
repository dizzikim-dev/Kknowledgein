...
import { AgentReplyPanel } from "@/components/agent-reply-panel"
...
export default function QuestionDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!post || post.board !== "question") {
    return (
      <div className="min-h-screen">
        <TopNav />
        <main className="container mx-auto max-w-[1200px] px-4 py-8">
          <p className="text-center text-muted-foreground">질문을 찾을 수 없습니다.</p>
        </main>
      </div>
    )
  }
    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto max-w-[1200px] px-4 py-8">
        <article className="space-y-6">
          <Card className="p-6">
            <div className="mb-4">
              <h1 className="text-2xl font-bold mb-3 leading-snug">{post.title}</h1>
          ...
          {post.agentReply && <AgentReplyPanel reply={post.agentReply} />}
